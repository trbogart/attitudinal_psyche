...
class TypingStats:
    ...
    def __init__(self, print_derived = True):
        self.print_derived = print_derived
        self.type_set = set(typing_videos)
        self.pair_count = 0
        self.pair_count_directional = 0

        self.missing_attitudes = {}
        for pos in range(1,5):
            for aspect in ['V', 'L', 'F', 'E']:
                attitude = f'{pos}{aspect}'
                self.missing_attitudes[attitude] = None
        for typings in self.type_set:
            for i, aspect in enumerate(typings):
                attitude = f'{i+1}{aspect}'
                if attitude in self.missing_attitudes:
                    del self.missing_attitudes[attitude]

        print('Missing attitudes:')
        ...
        self.print_blocks('Reactivist', 'F', 'E', derived=True)
        self.print_blocks('Evaluator', 'L', 'E', derived=True)
        self.print_blocks('Realist', 'L', 'F', derived=True)
        ...
        print(f'- {self.pair_count} of 18 unique function/block pairs')
        print(f'- {self.pair_count_directional} of 36 unique function/block pairs (directional)')

    def print_blocks(self, block, aspect1, aspect2, derived = False):
        if not derived or self.print_derived:
